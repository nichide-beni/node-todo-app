<template>
  <div class="container">
    <h1>TODO リスト</h1>
    <router-link to="/create" class="btn btn-primary">新しいTODOを作成</router-link>
    ...
    <ul v-else class="todo-list">
      <li v-for="todo in todos" :key="todo.id" :class="{ completed: todo.completed }">
        ...
        <span class="actions">
          <button @click="toggleStatus(todo)" :disabled="isDeleting" class="btn-action status-btn">
            {{ todo.completed ? '未完了にする' : '完了にする' }}
          </button>
          <router-link :to="{ name: 'TodoEdit', params: { id: todo.id } }" class="btn-action edit-btn">
            編集
          </router-link>
...
import { fetchTodos, updateTodoStatus, deleteTodo } from '@/controller/todoApi';
...
async function removeTodo(id) {
  ...
  try {
    ...
    // 成功したらリストから削除 (論理削除なので、IDがサーバー側で変わるわけではないが、表示から消す)
...
.status-btn { background-color: #28a745; }
